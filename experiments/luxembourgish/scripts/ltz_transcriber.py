"""Luxembourgish LV-ROVER transcriber - same method as competition_transcriber.py,
...
only language codes, canary set, tessdata dir, and PSM are Luxembourgish.

PSM is 7 (single line), not 3 (Hungarian, whole pages) or 6 (Maltese,
paragraph crops): the BnL eval items are already pre-cropped single lines."""
...
TESSDATA_DIR = "tessdata_ltz"

_TESS_LANG_PRIMARY = "ltz"
_TESS_LANG_AUGMENTED = "ltz+deu"
_TESS_LANG_ROMANCE = "ltz+deu+fra"
_TESS_LANG_STOCK = "ltzstock"
_ITA_FALLBACK_RATIO = 0.6
_CROSS_ENGINE_MAX_SWAP_DIST = 2

_MALTESE_DIAC = "àâäæçèéêëîïòóôöûüœÈ"  # Luxembourgish canary set, same role as ċġħż
_DIAC_FOLD = str.maketrans("àâäæçÈèéêëîïòóôöûü", "aaaaceeeeeiiooooou")
_PUNCT_STRIP = ".,;:!?\"'()[]{}«»—–-‘’“”"
_WS_SPLIT = re.compile(r"[ \t]+")
_DIC = pyphen.Pyphen(lang="de_DE")

with open("data/ltz_lexicon.json", encoding="utf-8") as f:
...
def _tess_paragraph(image: Image.Image, lang: str) -> str:
    raw = pytesseract.image_to_string(
        image, lang=lang, config=f'--tessdata-dir "{TESSDATA_DIR}" --psm 7'
    )
...
def transcribe(image: Image.Image, apply_postproc: bool = True) -> str:
    ...
    ltz_out = _tess_paragraph(image, _TESS_LANG_PRIMARY)
    ...
    tess_streams = [s for s in (ltz_out, aug_out, romance_out, stock_out, up_out) if s]
    if not tess_streams:
        return ""

    base = ltz_out if ltz_out else aug_out
