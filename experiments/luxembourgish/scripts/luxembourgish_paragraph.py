"""Luxembourgish paragraph renderer. Reuses the generic PIL render/justify/
augment primitives from the Maltese pipeline unchanged; no clitic/structural-
hyphen classification needed (verified this session: Luxembourgish hyphenation
is a plain typographic convention, same as Hungarian and German). Same method
(SynthTIGER-style render -> augment -> Tesseract fine-tune), different
language, per the explicit "no new architecture" constraint.

DPI=220 calibrated by directly measuring real BnL Luxembourgish line-crop
pixel heights (31-59px, mean 43px), not inferred from page metadata - the
mistake that cost three attempts on the Hungarian run.
"""
...
LTZ_CANARY = "àâäæçèéêëîïòóôöûüœÈ"
_DIC = pyphen.Pyphen(lang="de_DE")  # no Luxembourgish dictionary in pyphen; German
                                     # is the nearest available (verified this session)


@dataclass
class HuLayoutConfig:
    # Calibrated by direct measurement against real BnL line crops, not carried
    # over from another language's fix - see module docstring.
    dpi: int = 220
    font_pt_lo: int = 8
    font_pt_hi: int = 12
